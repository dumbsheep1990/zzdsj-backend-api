"""
核心业务逻辑层 - OWL集成管理
处理OWL框架集成的核心业务逻辑，包括配置管理、框架注册、插件管理等
"""
...
class OwlIntegrationManager:
    ...
    async def create_integration(
        ...
        Args:
            name: 集成名称
            framework_type: 框架类型（如agent, tool, workflow等）
            config: 配置信息
            description: 描述
            is_active: 是否活跃
            metadata: 元数据
        ...
        try:
            ...
            # 验证配置
            validation_result = await self._validate_config(framework_type, config)
            ...
            return {
                ...
                "data": {
                    ...
                    "framework_type": integration.framework_type,
    ...
    async def list_integrations(
        ...
        try:
            ...
            for integration in integrations:
                if framework_type and integration.framework_type != framework_type:
                    continue
    ...
    async def update_integration(
        ...
        try:
            ...
            if "config" in update_data:
                framework_type = update_data.get("framework_type", existing.framework_type)
                validation_result = await self._validate_config(framework_type, update_data["config"])
                if not validation_result["success"]:
                    return validation_result
    ...
    async def _validate_config(
        ...
        try:
            # 根据不同框架类型验证配置
            required_fields = {
                "agent": ["class_name", "module_path"],
                "tool": ["tool_class", "tool_module"],
                "workflow": ["workflow_config"],
                "plugin": ["plugin_name", "plugin_path"]
            }
            
            if framework_type not in required_fields:
                return {
                    "success": False,
                    "error": f"不支持的框架类型: {framework_type}"
                }
            
            # 检查必需字段
            for field in required_fields[framework_type]:
