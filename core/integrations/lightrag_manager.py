"""
核心业务逻辑层 - LightRAG集成管理
处理LightRAG集成的核心业务逻辑，包括配置管理、图谱构建、关系挖掘等
"""
...
class LightRAGIntegrationManager:
    """LightRAG集成管理器 - 核心业务逻辑层"""
    ...
    async def create_integration(
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"创建LightRAG集成配置失败: {str(e)}"
            }
    ...
    async def get_integration(self, integration_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"获取LightRAG集成配置失败: {str(e)}"
            }
    ...
    async def list_integrations(
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"获取LightRAG集成配置列表失败: {str(e)}"
            }
    ...
    async def update_integration(
        ...
        try:
            ...
            if "config" in update_data:
                validation_result = await self._validate_config(update_data["config"])
                if not validation_result["success"]:
                    return validation_result
            ...
            return {
                "success": False,
                "error": f"更新LightRAG集成配置失败: {str(e)}"
            }
    ...
    async def delete_integration(self, integration_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"删除LightRAG集成配置失败: {str(e)}"
            }
    ...
    def _hide_sensitive_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in sensitive_fields:
            ...
            # 处理嵌套配置
            if isinstance(hidden_config.get(field), dict):
                for nested_field in sensitive_fields:
                    if nested_field in hidden_config[field]:
                        hidden_config[field][nested_field] = "***"
    ...
    async def _validate_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # LightRAG的必需配置字段
            required_fields = ["working_dir", "llm_model", "embedding_model"]
            ...
            if isinstance(llm_config, dict) and "type" not in llm_config:
                return {
                    "success": False,
                    "error": "LLM模型配置缺少type字段"
                }
