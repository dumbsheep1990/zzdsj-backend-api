...
def test_direct_text_import():
    """直接测试text模块导入"""
    print("🔍 测试直接导入text模块...")
    
    try:
        # 直接导入text模块，避免其他模块的依赖问题
        from app.utils.text import (
            process_text, batch_process_texts, analyze_text_comprehensive,
            count_tokens, chunk_text, detect_language, extract_metadata_from_text,
            clean_text, extract_keywords, tokenize_text,
            TextLanguage, create_text_analyzer, create_token_counter, create_chunker
        )
...
def test_backward_compatibility():
    ...
    try:
        ...
        print(f"   🔑 关键词提取: {len(keywords)} 个关键词")
        
        # 分词接口
        tokens_list = tokenize_text(test_text)
        assert isinstance(tokens_list, list)
        print(f"   ✂️  文本分词: {len(tokens_list)} 个词")
...
def test_performance():
    ...
    try:
        ...
        texts = [f"测试文本 {i}" for i in range(20)]  # 减少数量避免过多日志
        ...
        assert batch_time < 3.0     # 批处理应该在3秒内完成
