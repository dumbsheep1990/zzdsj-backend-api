...
"""
智能内容处理功能数据库迁移脚本
支持MarkItDown、智能爬虫、内容质量分析等功能的数据库表创建
"""
...
# 导入数据库工具
from app.utils.core.database.connection_manager import DatabaseConnectionManager
...
class IntelligentContentMigration:
    """智能内容处理数据库迁移器"""
    
    def __init__(self):
        self.db_manager = DatabaseConnectionManager()
        self.migrations_dir = project_root / "migrations" / "sql" / "common"
...
async def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='智能内容处理数据库迁移脚本')
    parser.add_argument(
        '--verify-only', 
        action='store_true', 
    ...
    migrator = IntelligentContentMigration()
    
    try:
        if args.verify_only:
            # 仅验证
            logger.info("🔍 开始验证模式...")
            tables_ok = await migrator.verify_tables_created()
            indexes_ok = await migrator.verify_indexes_created()
        ...
        if args.test_configs:
            ...
            success = await migrator.test_portal_configs()
            sys.exit(0 if success else 1)
        
        # 执行迁移
        logger.info("🚀 开始执行智能内容处理数据库迁移...")
        migration_success = await migrator.execute_intelligent_content_migration()
        ...
        if migration_success:
            ...
            tables_ok = await migrator.verify_tables_created()
            indexes_ok = await migrator.verify_indexes_created()
            configs_ok = await migrator.test_portal_configs()
