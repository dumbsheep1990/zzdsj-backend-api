                ...
                for record in vector_records:
                    try:
                        delete_result = await vector_manager.delete_vector(
                            collection_name=record['vector_collection'] or 'default',
                            vector_id=record['vector_id']
                        )
                        if delete_result:
                            logger.info(f"成功删除向量: {record['vector_collection']}/{record['vector_id']}")
                        else:
                            logger.warning(f"向量可能已删除: {record['vector_id']}")
                    except Exception as e:
                        logger.error(f"删除向量{record['vector_id']}失败: {str(e)}")
                        deletion_success = False
                        
            except ImportError:
                # 如果向量管理器不可用，尝试使用Milvus客户端
                try:
                    ...
                    for record in vector_records:
                        try:
                            ...
                            logger.info(f"成功删除Milvus向量: {collection_name}/{vector_id}")
                        except Exception as e:
                            logger.error(f"删除Milvus向量{record['vector_id']}失败: {str(e)}")
                            deletion_success = False
                    ...
                    try:
                        ...
                        for record in vector_records:
                            try:
                                # 尝试从ES中删除向量文档
                                index_name = f"vectors_{record['vector_collection'] or 'default'}"
                                delete_result = await es_manager.delete_document(index_name, record['vector_id'])
                                
                                if delete_result.get('result') in ['deleted', 'not_found']:
                                    logger.info(f"成功删除ES向量: {index_name}/{record['vector_id']}")
                                else:
                                    logger.warning(f"ES向量删除结果未知: {record['vector_id']}")
                            except Exception as e:
                                logger.error(f"删除ES向量{record['vector_id']}失败: {str(e)}")
                                deletion_success = False
                                
                    except ImportError:
                        logger.warning("所有向量数据库客户端都不可用，跳过向量删除")
                        # 在生产环境中，这可能需要记录为严重错误
            
            logger.info(f"需要删除文档{file_id}的{len(vector_records)}个向量记录，删除成功：{deletion_success}")
