...
def get_advanced_retrieval_service(db: Optional[Session] = None) -> AdvancedRetrievalService:
    """获取高级检索服务实例"""
    global _advanced_retrieval_service
    if _advanced_retrieval_service is None:
        if db is None:
            raise ValueError("首次创建服务实例时必须提供 db 参数")
        _advanced_retrieval_service = AdvancedRetrievalService(db=db)
