...
async def list_assistants(
    ...
    return ResponseFormatter.format_success({"items": assistants, "total": total})
...
async def delete_assistant(assistant_id: int, db: Session = Depends(get_db)):
    ...
    return ResponseFormatter.format_success(None, message="助手已成功删除")
...
async def list_questions(
    ...
    return ResponseFormatter.format_success({"items": questions, "total": total})
...
async def get_question(
    ...
    return ResponseFormatter.format_success(question)
...
async def create_question(question: QuestionCreate, db: Session = Depends(get_db)):
    ...
    return ResponseFormatter.format_success(created)
...
async def delete_question(question_id: int, db: Session = Depends(get_db)):
    ...
    if not success:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="删除问题失败")
    
    return ResponseFormatter.format_success(None, message="问题已成功删除")
...
async def answer_question(question_id: int, db: Session = Depends(get_db)):
    ...
    return ResponseFormatter.format_success({"answer": answer})
        ...
        for tool_config in tools:
            if tool_config.tool_type == "mcp":
                ...
                if mcp_tool:
                    ...
                    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"未找到MCP工具: {tool_config.tool_name}")
                ...
                # 未知工具类型
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"不支持的工具类型: {tool_config.tool_type}")
        
        # 更新助手工具配置
        manager.update_assistant_tools(assistant_id, configured_tools)
        
        return ResponseFormatter.format_success(configured_tools)
        
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"配置助手工具时出错: {str(e)}")
...
async def get_assistant_tools(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取助手工具配置时出错: {str(e)}") 