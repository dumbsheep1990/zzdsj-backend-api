-- =====================================================
-- ZZ-Backend-Lite 数据库补充脚本
-- 基于Schema层与Model层对比分析
-- 补充缺少的表和修复字段不匹配问题
-- =====================================================
...
-- 补充context_compression_executions表字段（如果表存在）
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='context_compression_executions') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='context_compression_executions' AND column_name='strategy_id') THEN
            ALTER TABLE context_compression_executions ADD COLUMN strategy_id VARCHAR(36) REFERENCES compression_strategies(id);
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='context_compression_executions' AND column_name='compression_ratio') THEN
            ALTER TABLE context_compression_executions ADD COLUMN compression_ratio FLOAT;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='context_compression_executions' AND column_name='quality_score') THEN
            ALTER TABLE context_compression_executions ADD COLUMN quality_score FLOAT;
        END IF;
    END IF;
END $$;

-- 补充owl_toolkits表字段（如果表存在）
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='owl_toolkits') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='owl_toolkits' AND column_name='version') THEN
            ALTER TABLE owl_toolkits ADD COLUMN version VARCHAR(20) DEFAULT '1.0.0';
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='owl_toolkits' AND column_name='config') THEN
            ALTER TABLE owl_toolkits ADD COLUMN config JSONB DEFAULT '{}';
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='owl_toolkits' AND column_name='tags') THEN
            ALTER TABLE owl_toolkits ADD COLUMN tags JSONB DEFAULT '[]';
        END IF;
    END IF;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '数据库补充脚本执行完成！';
    RAISE NOTICE '新增表数量: 15个';
    RAISE NOTICE '修复字段不匹配: 8处';
    RAISE NOTICE '新增权限: 8个';
